...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Retrieve the 'user' and 'success' parameters
  const userParams = urlParams.get("user");
  ...
  if (sessionStorage.user_data) {
    const userData = JSON.parse(sessionStorage.user_data);
    if (userData.token) {
      token = userData.token;
      console.log("Token (from sessionStorage user_data):", token);
    }
  } else {
    console.log("Token not found in sessionStorage user_data.");
  }

  // Now, 'token' holds the JWT token value
  console.log("Token:", token);
  // ... (the rest of your code for parsing user data)
});
...
// Define the token variable at the global scope

// Example: Get the token from sessionStorage
...
function uploadFile(Id) {
  ...
  console.log("uploadFile function is working now line 218");
  var userIds = Id;
